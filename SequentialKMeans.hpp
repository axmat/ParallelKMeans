...
template <typename T>
auto SeqInitClusters(const std::vector<Point<T>> &data,
...
// Assignment step
template <typename T>
std::size_t SeqAssignPoints(std::vector<Point<T>> &data,
...
// Update the clusters
template <typename T>
void SeqUpdateClusters(const std::vector<Point<T>> &data,
                       std::vector<Cluster<T>> &clusters) {
  for (auto &C : clusters) {
    C.Update(data);
    C.Clear();
...
SeqKMeans(std::vector<Point<T>> &data, const std::size_t &nClusters,
